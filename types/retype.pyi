...
def get_offset_and_prefix(body: Node, skip_assignments: bool) -> Tuple[int, str]: ...
def get_annotated_param(node: _LN, arg: ast3.arg, *, missing_ok: bool = ...) -> _LN: ...
...
def parse_signature_type_comment(
    type_comment: str
...
def annotate_parameters(
    parameters: _LN, ast_args: ast3.arguments, *, is_method: bool
) -> None: ...
def append_after_imports(stmt_to_insert: Node, node: Node) -> None: ...
def make_import(*names: ast3.alias, from_module: Optional[str] = ...) -> Node: ...
...
def retype_file(
    src: Path, pyi_dir: Path, targets: Path, *, quiet: bool = ..., hg: bool
) -> Path: ...
def retype_path(
    src: Path,
    pyi_dir: Path,
